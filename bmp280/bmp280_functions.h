...
const float EXP_FILTER_COEF_PRESSURE = 0.985;
...
extern float altitudeBMP;
extern unsigned long int pressureBMP;
extern float slowPressure, fastPressure;

unsigned long int rawPressure;
double altitudeCalibration;

const int numSamples = 20;
unsigned long int sensorDatas[numSamples], sumSensorData;
int memoryPosition;
...
void getAltitude(bool afterCalibration = true){
  if(memoryPosition == numSamples) memoryPosition = 0;
  sumSensorData -= sensorDatas[memoryPosition];
  sensorDatas[memoryPosition] = rawPressure;
  sumSensorData += sensorDatas[memoryPosition];
  ++memoryPosition;

  fastPressure = sumSensorData / numSamples;
  if(afterCalibration)
    slowPressure = (EXP_FILTER_COEF_PRESSURE * slowPressure) + (1.0 - EXP_FILTER_COEF_PRESSURE) * fastPressure;
  else slowPressure = fastPressure;

  float error = fastPressure - slowPressure;
  if(abs(error) > 6.0)
    slowPressure += error;
  
  altitudeBMP = 44330.0 * (1 - pow((double)slowPressure / 101325.0, 1 / 5.255)); //altitude meters
  if(afterCalibration)
    altitudeBMP -= altitudeCalibration;
}
...
void BMP280_calibrate(){
  ...
  for(int i = 0; i < numIteration / 10; ++i){
    getBMPData(); getAltitude(false);
    delay(10);
  }
  for(int i = 0; i < 500; ++i){
    getBMPData();
    getAltitude(false);
    altitudeCalibration += altitudeBMP;
    delay(10);
  } 
  altitudeCalibration /= numIteration;
