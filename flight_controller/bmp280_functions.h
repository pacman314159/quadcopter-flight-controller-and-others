...
extern double altitudeBMP;
double altitudeBMPCalibration;
#define ALPHA 0.985//COMPLEMENTARY FILTER
#define NUM_SAMPLES 20

void getBMPAltitude(bool afterCalibration = true){
   ...
   altitudeBMP = 44330.0 * (1 - pow((double)p / 101325.0, 1 / 5.255));
   if(afterCalibration)
      altitudeBMP -= altitudeBMPCalibration;
}
...
void BMP280_calibrate(){
   int calibrationNumber = 300;
   Serial.println("START CALIBRATING BMP280");
   for(int i = 0; i < calibrationNumber; i++){
      getBMPAltitude(false);
      altitudeBMPCalibration += altitudeBMP;
      delay(10);
   }
   altitudeBMPCalibration /= calibrationNumber;
   Serial.print("BMP280 CALIBRATION DONE");
}
