...
void setup(){
  ...
  DDRB |= B00100001; PORTB |= B00100001; //Declare D13, D8 output then set HIGH both
  DDRD |= B10100000; PORTD |= B00100000; //Declare D5, D7 output then set HIGH D5 for magnet
  ...
  finalPause();
  PORTB &= B11011110; //set D8, D13 low
  // MPU6050_calibrate();
  delay(1000);
...
void loop(){
  ...
  if(micros() - previousTime >= loopTimeMicros){ // if 200Hz loop is overwhelmed
    PORTD &= B01111111;//turn off D7
    PORTB |= B00100000;//turn on D13
  }
